      ...
      <Page testID="screen-calendar">
        ...
        {mode === 'month' ? (
          <Card style={styles.calendarCard}>
            ...
            <View style={styles.monthGrid}>{cells.map((date) => { const key = dateKey(date); const isSelected = key === selected; const isToday = key === today; const dim = date.getMonth() !== selectedDate.getMonth(); const count = selectCalendarEntriesForDate(store.data, key).length; return <Pressable key={key} testID={`calendar-day-${key}`} accessibilityRole="button" accessibilityLabel={`${formatShortDate(date)}，${count}项任务`} onPress={() => setSelected(key)} style={[styles.day, isSelected && styles.daySelected, isToday && !isSelected && styles.dayToday]}><Text style={[styles.dayText, dim && styles.dayDim, isSelected && styles.dayTextSelected]}>{date.getDate()}</Text>{count ? <View style={[styles.eventDot, isSelected && styles.eventDotSelected]} /> : null}</Pressable>; })}</View>
          </Card>
