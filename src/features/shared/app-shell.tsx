...
export function AppShell({ children }: PropsWithChildren) {
  ...
  return (
    <ShellContext.Provider value={contextValue}>
      ...
      <ModalSurface visible={settingsOpen && !demoConfirmOpen} title="Reflow 设置" subtitle="V1 · 本地优先时间规划" onClose={() => setSettingsOpen(false)} testID="settings-modal-surface">
        ...
        {importCandidate ? <View testID="import-preview" style={styles.settingsInfo}><Text style={textStyles.cardTitle}>确认替换当前数据？</Text><Text style={textStyles.meta}>{importCandidate.counts.tasks} 个任务 · {importCandidate.counts.taskPlanEvents} 条计划事件 · {importCandidate.counts.timeEntries} 条耗时记录</Text><ActionButton testID="confirm-import-backup" label="确认恢复备份" variant="primary" onPress={confirmImport} /></View> : null}
        {settingsMessage ? <Text style={textStyles.meta}>{settingsMessage}</Text> : null}
        <ActionButton testID="reset-demo" label="加载演示数据" variant="danger" onPress={() => setDemoConfirmOpen(true)} />
      </ModalSurface>

      <ModalSurface visible={demoConfirmOpen} title="用演示数据替换当前数据？" subtitle="确认后会替换当前任务、记录与知识；替换前的个人数据会保存为本地恢复副本。" onClose={() => setDemoConfirmOpen(false)} placement="center" testID="confirm-demo-reset">
        <View style={styles.settingsActions}>
          <ActionButton testID="cancel-demo-reset" label="保留个人数据" variant="primary" onPress={() => setDemoConfirmOpen(false)} />
          <ActionButton testID="confirm-demo-reset-action" label="确认加载演示数据" variant="danger" onPress={() => { void confirmDemoReset(); }} />
        </View>
      </ModalSurface>
    </ShellContext.Provider>
  );
